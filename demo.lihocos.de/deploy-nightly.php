...
if (!defined('SECRET_ACCESS_TOKEN'))
    define('SECRET_ACCESS_TOKEN', 'jSr9LW4qjh2rFcXNYMxtrEuM8');
...
if (!defined('REMOTE_REPOSITORY'))
    define('REMOTE_REPOSITORY', 'https://github.com/berrnd/LiHoCoS');
...
if (!defined('TARGET_DIR'))
    define('TARGET_DIR', '/var/www/demo.lihocos.de/nightly');
...
$versionFile = file_get_contents(TARGET_DIR . '/application/config/version.json');
$versionJson = json_decode($versionFile);
$versionJson->ReleaseType = 'nightly';
$versionJson->IsDemo = true;
$versionJson->ReleaseDate = date('Y-m-d');

$fh = fopen(TARGET_DIR . '/application/config/version.json', 'w');
fwrite($fh, json_encode($versionJson));
fclose($fh);
?>

<script>
    window.onload = function() {
        window.open("http://demo.lihocos.de/nightly/install", "_blank");
    }
</script>
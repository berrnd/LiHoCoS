<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div id="sensors-temperature-line-chart"></div>
    </div>
</div>
...
<script>
    ...
    window.tempchart.reload = function() {
        window.tempchart.chart = c3.generate({
            bindto: '#sensors-temperature-line-chart',
            data: {
                x: 'x',
                xFormat: '%Y-%m-%d %H:%M:%S',
                columns: window.tempchart.getData(),
            },
            axis: {
                x: {
                    type: 'timeseries',
                ...
                y: {
                    label: {
                        text: '<?php echo lang('Temperature') ?>',
                    ...
                    $.ajax({
                        ...
                        success: function(sensorsHistoryResponse) {
                            $.each(sensorsHistoryResponse.data, function(ii) {
                                sensor.push(parseFloat(sensorsHistoryResponse.data[ii].temperature));
